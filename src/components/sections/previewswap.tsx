/* SAME as Preview component, but GIF and image previews are swapped! */
...
export class PreviewSwap extends React.Component<PreviewProps> {
  render() {
    return (
      <>
        ...
        <div className="previewWrapper">
          ...
          <div className="previewGifContainer">
            <img
              ...
              src={`/${this.props.slug}-mobile-preview.gif`}
            ...
            <div className={`previewImgContainer ${this.props.numImgs}`}>
              {this.props.imgAlts.slice(0, -1).map((alt: string, i: number) => (
                <img
                  ...
                  src={`/${this.props.slug}-mobile-screenshot-${i + 1}.png`}
                  alt={`${alt}[${i}]`}
                  tabIndex={0}
                />
              ))}
              {this.props.imgAlts.slice(-1).map((alt: string, i: number) => (
                <img
                  src={`/${this.props.slug}-mobile-screenshot-${this.props.imgAlts.length}.png`}
                  alt={`${alt}[${i}]`}
                  tabIndex={0}
                />
              ))}
